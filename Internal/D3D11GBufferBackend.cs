...
internal readonly struct GBufferVertex
{
    ...
    public GBufferVertex(NumericsVector3 position, NumericsVector3 normal, NumericsVector4 color, NumericsVector2 texCoord)
    {
        Position = position;
        Normal = normal;
        Color = color;
...
internal sealed class GBufferFrame
{
    ...
    public GBufferFrame(GBufferDrawCommand[] commands, GBufferMaterial material, SemitransparentLighting lighting, long cycleId)
    {
        Commands = commands;
        Material = material;
        Lighting = lighting;
...
internal sealed unsafe class D3D11GBufferBackend : IDisposable
{
    ...
    private sealed class PendingTargets : IDisposable
    {
        ...
        public PendingTargets(GBufferTarget target, nint[] renderTargets, nint depthStencil, uint width, uint height)
        {
            Target = target;
    ...
    public D3D11GBufferBackend(IGameInteropProvider gameInteropProvider, IPluginLog log)
    {
        ...
        using var compiledSemitransparentPixelShader = ShaderBytecode.Compile(shaderSource, "PSSemitransparent", "ps_5_0");
        ...
        semitransparentPixelShader = new PixelShader(device, compiledSemitransparentPixelShader.Bytecode);
        semitransparentCompositePixelShader = new PixelShader(device, compiledSemitransparentCompositePixelShader.Bytecode);
        ...
        compositeBlendDescription.RenderTarget[0].DestinationAlphaBlend = BlendOption.InverseSourceAlpha;
        ...
        semitransparentDepthTestWriteStencilState = new DepthStencilState(device, semitransparentDepthDescription);

        semitransparentDepthDescription.IsDepthEnabled = true;
        semitransparentDepthDescription.DepthWriteMask = DepthWriteMask.Zero;
        semitransparentDepthDescription.DepthComparison = Comparison.GreaterEqual;
        semitransparentDepthDescription.IsStencilEnabled = false;
        semitransparentCompositeDepthState = new DepthStencilState(device, semitransparentDepthDescription);
    ...
    public void Publish(GBufferTarget target, List<GBufferDrawCommand> commands, GBufferMaterial material, SemitransparentLighting lighting)
    {
        GBufferFrame? oldFrame;
        lock (stateLock)
        {
            var newFrame =
                commands.Count == 0 ? null : new GBufferFrame([.. commands], material, lighting, Interlocked.Increment(ref nextCycleId));
    ...
    )
    {
        ...
        try
        {
            if (!detouring && context == immediateContextPointer && numRenderTargetViews != uint.MaxValue)
            {
                lock (stateLock)
                {
                    pending = TrackTargetChange(numRenderTargetViews, renderTargetViews, depthStencilView);
                }
            }
        }
        catch (Exception exception)
        {
            log.Warning(exception, "[Underpaint] Failed to track opaque G-buffer targets with UAVs.");
        }
    ...
    {
        if (detouring || context != immediateContextPointer || !semitransparentCompositeArmed || semitransparentCompositeInjected)
        {
            return;
        }
        ...
        try
        {
            lock (stateLock)
            {
                ...
                var renderTargets = immediateContext.OutputMerger.GetRenderTargets(2, out var depthStencil);
                var shaderResources = immediateContext.PixelShader.GetShaderResources(0, 16);
                try
                {
                    if (renderTargets.Length < 2 || renderTargets[0] == null || renderTargets[1] != null || depthStencil == null)
                    {
                        return;
                    }

                    var (lightDiffuse, lightSpecular) = FindSemitransparentLightViews(shaderResources);
    ...
    )
    {
        var constants = new WorldConstants
        {
            ...
            SemitransparentLighting = new Vector4(frame.Lighting.Ambient, frame.Lighting.Diffuse, frame.Lighting.Specular, 0f),
        };
        ...
        try
        {
            ...
            deferredContext.OutputMerger.SetDepthStencilState(semitransparentCompositeDepthState, 0);
            ...
            deferredContext.InputAssembler.SetVertexBuffers(0, new VertexBufferBinding(vertexBuffer, Utilities.SizeOf<GBufferVertex>(), 0));
            ...
            foreach (var command in frame.Commands)
            {
                ...
                deferredContext.MapSubresource(vertexBuffer, MapMode.WriteDiscard, MapFlags.None, out var vertexStream);
                try
                {
                    fixed (GBufferVertex* vertices = command.Vertices)
                    {
                        vertexStream.Write((nint)vertices, 0, Utilities.SizeOf<GBufferVertex>() * command.Vertices.Length);
                    }
                }
                finally
                {
                ...
                deferredContext.PixelShader.SetShaderResource(0, command.Texture ?? whiteTextureView);
            ...
            if (!semitransparentCompositeDrawLogged)
            {
                ...
                log.Information($"[Underpaint] Semitransparent composite enabled: target={targetFormat}, size={width}x{height}.");
            }
        }
        finally
        {
    ...
    private static (ShaderResourceView? Diffuse, ShaderResourceView? Specular) FindSemitransparentLightViews(ShaderResourceView[] views)
    {
    ...
    private void PSSetShaderResourcesDetour(nint context, uint startSlot, uint numViews, nint* shaderResourceViews)
    {
        ...
        {
            log.Warning(exception, "[Underpaint] Failed to identify the semitransparent composite pass.");
        }
        finally
        {
    ...
    private static bool BindsBothSemitransparentLightBuffers(uint numViews, nint* views)
    {
        ...
        for (var slot = 0; slot < numViews; slot++)
        {
            ...
            hasDiffuse |= diffuseTexture != null && resource == (nint)diffuseTexture->D3D11Texture2D;
            hasSpecular |= specularTexture != null && resource == (nint)specularTexture->D3D11Texture2D;
        }

        return hasDiffuse && hasSpecular;
    }

    private PendingTargets? TrackTargetChange(uint numViews, nint* renderTargetViews, nint depthStencilView)
    {
        PendingTargets? pending = null;
        ...
        {
            var frame = candidateTarget == GBufferTarget.Opaque ? opaqueFrame : semitransparentFrame;
    ...
    private void IssueSafely(PendingTargets? pending)
    {
        ...
        try
        {
            lock (stateLock)
            {
                var frame = pending.Target == GBufferTarget.Opaque ? opaqueFrame : semitransparentFrame;
                if (frame != null)
                {
    ...
    private void Issue(PendingTargets pending, GBufferFrame frame)
    {
        ...
        var constants = new WorldConstants
        {
            ...
            DitherParameters = new Vector4(currentDitherPhase, pending.Target == GBufferTarget.Semitransparent ? 1f : 0f, 0f, 0f),
        };
        ...
        try
        {
            ...
            var selectedDepthStencilState =
                pending.Target == GBufferTarget.Semitransparent ? semitransparentDepthTestWriteStencilState : depthStencilState;
            var stencilReference = pending.Target == GBufferTarget.Semitransparent ? 0x10 : frame.Material.Stencil;
            deferredContext.OutputMerger.SetDepthStencilState(selectedDepthStencilState, stencilReference);
            ...
            deferredContext.VertexShader.Set(vertexShader);
            deferredContext.VertexShader.SetConstantBuffer(0, constantsBuffer);
            deferredContext.PixelShader.Set(pending.Target == GBufferTarget.Semitransparent ? semitransparentPixelShader : pixelShader);
            deferredContext.PixelShader.SetConstantBuffer(0, constantsBuffer);
            deferredContext.PixelShader.SetSampler(0, sampler);
            deferredContext.UpdateSubresource(ref constants, constantsBuffer);
            ...
            foreach (var command in frame.Commands)
            {
                if (command.Vertices.Length > MaxVerticesPerCommand)
                {
                    log.Warning($"[Underpaint] Opaque command has {command.Vertices.Length} vertices; maximum is {MaxVerticesPerCommand}.");
                    continue;
                }
    ...
    private (Texture2D Texture, ShaderResourceView View) CreateWhiteTexture()
    {
        ...
        var texture = new Texture2D(device, description, [new DataRectangle((nint)(&pixel), sizeof(uint))]);
        return (texture, new ShaderResourceView(device, texture));
    }

    private (int MatchedCount, uint Width, uint Height) MatchFullGBuffers(uint numViews, nint* renderTargetViews, GBufferTarget target)
    {
        var manager = RenderTargetManager.Instance();
        ...
        for (var slot = 0; slot < expectedTargetCount; slot++)
        {
            ...
            var texture = target == GBufferTarget.Opaque ? manager->GBuffers[index].Value : manager->SemitransparentGBuffers[index].Value;
            ...
            if (
                expectedView == 0
                || (renderTargetViews[slot] != expectedView && GetViewResource(renderTargetViews[slot]) != (nint)texture->D3D11Texture2D)
            )
            {
                continue;
            }
    ...
    private static KernelTexture* GetSemitransparentLightTexture(RenderTargetManager* manager, bool specular) =>
    ...
    private static Vector4 ToSharpDx(NumericsVector4 value) => new(value.X, value.Y, value.Z, value.W);
}
